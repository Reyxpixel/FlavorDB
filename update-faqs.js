...
const content = fs.readFileSync('client/src/components/FAQPage.jsx', 'utf8');
let replaced = content.replace(/const faqs = \[[\s\S]*?\];\s+return/m, 'const faqs = ' + JSON.stringify(newFaqs, null, 2) + ';\n\n  return');

// Add the wrapper for CSS grid animation
replaced = replaced.replace(/<div className="legacy-panel-collapse">\s*<div\s*className="legacy-panel-body"\s*dangerouslySetInnerHTML=\{\{ __html: faq\.bodyHtml \}\}\s*\/>\s*<\/div>/g, 
...
// Remove {openPanel === faq.id && ( ... )} conditional rendering for CSS transition
replaced = replaced.replace(/\{openPanel === faq\.id && \(\s*(<div className={`legacy-panel-collapse \$\{openPanel === faq\.id \? "open" : ""\}`\}>(?:[\s\S]*?)<\/div>\s*<\/div>\s*<\/div>)\s*\)\}/g, '$1');

fs.writeFileSync('client/src/components/FAQPage.jsx', replaced);
